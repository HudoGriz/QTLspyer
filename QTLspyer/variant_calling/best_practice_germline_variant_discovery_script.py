...
parser.add_argument('--experimentName', '-exn', help='Name of the experiment (for file naming).', type=str)

parser.add_argument('--Reference', '-r', help='Name of the reference fasta file.', type=str)
parser.add_argument('--ReferenceName', '-rn', help='Name of the reference organism.', type=str)
parser.add_argument('--Adapters', '-ad', help='File name with adapters.', type=str)
parser.add_argument('--ReferenceVCF', '-rvcf', help='File name of reference VCF file.', type=str)

parser.add_argument('--SeqSingle', '-ss', help='The sample sequences single.',  default="FALSE", type=str)

parser.add_argument('--runFastqcPreTrim', default=0, help='Run FastQC on reads before trimming.', type=int)
parser.add_argument('--runBBduk', default=0, help='Run BBduk for read trimming.', type=int)
parser.add_argument('--runFastqcPostTrim', default=0, help='Run FastQC on reads after trimming.', type=int)

parser.add_argument('--Trimming', '-t', help='Pipeline to include trimming.', default=0, type=int)
parser.add_argument('--Mapping', '-map', help='Pipeline to include mapping.', default=0, type=int)
parser.add_argument('--MarkingDuplicates', '-md', help='Pipeline to include marking duplicates.', default=0, type=int)
parser.add_argument('--ReadGroupsProcessing', '-rgp', help='Pipeline to include read grouping.', default=0, type=int)
parser.add_argument('--BaseRecalibration', '-brec', help='Pipeline to include base recalibration.', default=0, type=int)
parser.add_argument('--VariantCalling', '-vc', help='Pipeline to include variant calling.', default=0, type=int)
parser.add_argument('--CombineGVCFs', '-cg', help='Pipeline to include variant calling.', default=0, type=int)
parser.add_argument('--FilteringOutSNPs', '-fos', help='Pipeline to include filtering out SNPs.', default=0, type=int)
parser.add_argument('--VariantsToTable', '-vtt', help='Pipeline to include .vcf to table transformation.', default=0,
                    type=int)

# Toll settings
## BBduk
parser.add_argument('--Bbduk_n_cores', '-BB_cor', help='Cores BBduk.sh should use when trimming.', default=8, type=int)
...
for sample in qtl.ori_samples:
    ...
    if args.SeqSingle == "TRUE":
        ...
        if do_fastq_pretrimm == 1:
            qtl.fastqc_single(status='untrimmed', sample=sample, in1=f'{qtl.local_input}/{r}')
        ...
        if do_fastq_pretrimm == 1:
            qtl.fastqc(status='untrimmed', sample=sample, in1=f'{qtl.local_input}/{r1}', in2=f'{qtl.local_input}/{r1}')
        ...
        if do_fastq_trimm == 1:
            qtl.fastqc(status='trimmed', sample=sample, in1=trimmed[0], in2=trimmed[1])

        mapped = qtl.bwa(sample=sample, reference=fasta_ref_file, in1=trimmed[0], in2=trimmed[1], exe=args.Mapping,
                         core=args.Bbduk_n_cores)

    qtl.samtools_index(sample=sample, in1=mapped, exe=args.Mapping)

    mapped = qtl.picard_md(sample=sample, in1=mapped, exe=args.MarkingDuplicates)

    mapped = qtl.picard_rg(sample=sample, in1=mapped, ref_organism=ref_org, exe=args.ReadGroupsProcessing)

    mapped = qtl.gatk_BaseRecalibrator(sample=sample, bam=mapped, ref_fasta=fasta_ref_file, exe=args.BaseRecalibration,
...
vcfs = qtl.gatk_merge_vcfs(exe=args.CombineGVCFs, reference=fasta_ref_file, sample=args.experimentName)

filtered_variants = qtl.gatk_variant_selection(sample=args.experimentName, in1=vcfs, reference=fasta_ref_file,
                                               out=f'output/GATK/nonfiltered/{args.experimentName}.snps.vcf',
                                               parameters='SNP',
                                               exe=args.FilteringOutSNPs)

variant_table = qtl.gatk_varints_to_table(sample=args.experimentName, in1=filtered_variants, exe=args.VariantsToTable)
