...
for sample in qtl.ori_samples:
    ...
    if args.SeqSingle == "TRUE":
        ...
        mapped = qtl.bwa(
            sample=sample, reference=fasta_ref_file, in1=trimmed[0], in2=trimmed[1], core=args.Bbduk_n_cores, exe=args.Mapping
        )

    read_groups = qtl.picard_rg(sample=sample, in1=mapped, ref_organism=ref_org, exe=args.ReadGroupsProcessing)

    marked_duplicates = qtl.picard_md(sample=sample, in1=read_groups, exe=args.MarkingDuplicates)

    qtl.samtools_index(sample=sample, in1=marked_duplicates, exe=args.MarkingDuplicates)
...
selected_variants = qtl.gatk_variant_selection(
    ...
    out=f'{qtl.doc_output}/GATK/nonfiltered/{args.experimentName}.snps.vcf',
    parameters='SNP', exe=args.FilteringOutSNPs
)

variant_table = qtl.gatk_varints_to_table(sample=args.experimentName, in1=selected_variants, exe=args.VariantsToTable)
